...
from src.schemas.signals import FilingCitation, Recommendation, WebSourceCitation
...
def _agent_card(signal) -> AgentSignalCard:
    ...
    for factor in getattr(signal, "key_factors", []) or []:
        cleaned = clean_reasoning_text(str(factor), limit=160)
        if cleaned:
...
def build_signal_cards(recommendation: Recommendation) -> list[SignalCard]:
    ...
    for ticker in recommendation.tickers:
        ...
        if decision is None:
            continue
        agents = [_agent_card(signal) for signal in by_ticker.get(ticker, [])]
        card = SignalCard(
...
def signal_cards_context_text(recommendation: Recommendation) -> str:
    ...
    for card in build_signal_cards(recommendation):
        ...
        lines.extend([
            "",
            f"{card.ticker}: {card.action.upper()} at {card.action_confidence:.0%}; consensus {card.consensus_direction.upper()} at {card.consensus_confidence:.0%}; weighted score {card.weighted_score:+.2f}.",
